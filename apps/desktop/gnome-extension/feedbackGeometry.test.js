...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Otter Code");
  expect(findCaptureDestination([window(99, "Otter Code"), target], 42, "Otter Code")).toBe(target);
  expect(findCaptureDestination([window(99, "Otter Code")], 42, "Otter Code")).toBeUndefined();
