...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/Otter Code (Nightly).app/Contents/MacOS/Otter Code",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/Otter Code.app/Contents/MacOS/Otter Code")).toBe(
      "/Applications/Otter Code.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/Otter Code.app/other/MacOS/Otter Code")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/Otter Code (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/Otter Code (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged Otter Code icon is missing");
  expect(windows).toHaveLength(0);
});
