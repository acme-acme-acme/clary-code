...
export async function installGnomeCaptureBundle({ bundle, dataHome }: SetupPaths) {
  ...
  if (existing) {
    ...
    if (installed.version > metadata.version)
      throw new Error("A newer extension is installed. Update Otter Code instead of replacing it.");
  }
    ...
    try {
      ...
      if (installed && (!info.state || (info.version && installed.version > info.version.value)))
        return {
          ...
          message:
            "Installed. Save your work, sign out of GNOME and sign back in, then return here to enable the extension. Restarting Otter Code alone is not enough.",
        };
      ...
      if (!properties.UserExtensionsEnabled.value)
        return {
          ...
          message:
            "GNOME has disabled user extensions. Turn on Extensions in the GNOME Extensions app, then check again. Otter Code will not enable your other extensions for you.",
        };
      if (info.state?.value === 1)
        return {
          status: "enabled",
          message: "The Otter Code extension is running. Active-window snapshots are available.",
        };
      ...
      return {
        status: "disabled",
        message:
          "Enable the Otter Code extension to allow active-window snapshots. You can disable it here at any time.",
      };
    } catch (error) {
      return {
        status: "error",
