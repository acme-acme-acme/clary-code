 ...
 * every `@otterware/otter-code-<platform>.tgz` first, `otter-code.tgz` (the launcher) last, so
...
const publishCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const scopeDir = path.join(packagesDir, "@otterware");
      const launcherTarball = path.join(packagesDir, "otter-code.tgz");
      const platformTarballs = (yield* fs
        .readDirectory(scopeDir)
        .pipe(Effect.orElseSucceed((): ReadonlyArray<string> => [])))
        .filter((entry) => entry.startsWith("otter-code-") && entry.endsWith(".tgz"))
        .sort()
        .map((entry) => path.join(scopeDir, entry));
      if (platformTarballs.length === 0) {
        return yield* new ServerCliBuildAssetMissingError({
          assetPath: path.join(scopeDir, "otter-code-<platform>.tgz"),
        });
      }
  ...
  Command.withDescription(
    "Publish the @otterware/otter-code-<platform> tarballs and then the otter-code launcher to npm.",
  ),
);
