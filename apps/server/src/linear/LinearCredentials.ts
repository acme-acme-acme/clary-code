...
export const makeLinearCredentials = Effect.gen(function* () {
  ...
  const current: Effect.Effect<LinearCredential | null> = Effect.gen(function* () {
    ...
    if (cached.token !== null) return { kind: "user", token: cached.token };
    const apiKey = yield* settings.getSettings.pipe(
      Effect.map((value) => value.linear.apiKey),
      Effect.orElseSucceed(() => ""),
    );
    return apiKey.length > 0 ? { kind: "apiKey", token: apiKey } : null;
  });
