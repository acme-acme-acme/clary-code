...
/**
 * Active issues are re-read this often. Linear's live feed triggers a sweep as
 * soon as a linked issue changes; this is the fallback when the feed is down.
 */
...
export const make = Effect.gen(function* () {
  ...
  const liveFeed = yield* makeLinearLiveFeed(
    credentials.current.pipe(Effect.map((credential) => credential?.token ?? null)),
  );
  ...
  const readIssueDetail: LinearIssueSyncReactor["Service"]["readIssueDetail"] = Effect.fn(
    ...
    if (credential === null) {
      return yield* new LinearIssueDetailError({
        ...
        detail:
          "Link your Linear account (Settings → Connections → Linear agent) or add a Linear API key (Settings → Integrations → Linear) to see issue details.",
      });
    }
