  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Otter Connect.`,
      });
      return true;
    }
  ...
  return visibleEnvironments.map(({ environment, availability, error, status }) => {
    ...
    const statusText =
      unsupported && !savedEnvironment
        ? "Otter Connect · Not added · Client not supported"
        : savedConnection
          ? savedConnection.statusText
          : availability === "online"
            ? "Otter Connect · Not added · Relay online"
            : availability === "offline"
              ? "Otter Connect · Not added · Relay offline"
              : availability === "checking"
                ? "Otter Connect · Not added · Checking relay status…"
                : (Option.getOrNull(error)?.message ??
                  "Otter Connect · Not added · Relay status unavailable");
    if (selection) {
      return (
        <label
