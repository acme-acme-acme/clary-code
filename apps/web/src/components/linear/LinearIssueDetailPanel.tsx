      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="flex flex-col gap-5 px-4 pt-2 pb-6">
          ...
          <Section title="Activity">
            ...
            <IssueActivity threadRef={threadRef} activity={detail.activity} />
          </Section>
        </div>
      </ScrollArea>
  ...
  return (
    <ol className="flex flex-col gap-3">
      {items.map((item) =>
        item.kind === "thread" ? (
          <li key={item.root.id}>
            <CommentThread threadRef={threadRef} root={item.root} replies={item.replies} />
          </li>
        ) : (
          <li
            key={item.entry.id}
        ...
        return (
          <div key={comment.id}>
            ...
            <div className="px-3 py-2.5">
              ...
              <div className={leads ? "text-[0.95rem]" : "pl-6"}>
                <ChatMarkdown
                  text={comment.body}
                  cwd={undefined}
                  threadRef={threadRef}
    ...
    <Empty className="min-h-0 justify-center-safe">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        ...
        <Button
          size="sm"
          variant="ghost"
          onClick={() => void navigate({ to: "/settings/integrations", hash: "linear-api-key" })}
        >
          <SettingsIcon />
          Linear settings
        </Button>
