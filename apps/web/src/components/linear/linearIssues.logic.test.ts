...
import {
  ...
  resolveLinkLinearIssueInput,
  resolveThreadLinearIssueBadge,
} from "./linearIssues.logic";
...
describe("linearIssueStateColor", () => {
  it("uses Linear's hex color and ignores anything else", () => {
    expect(linearIssueStateColor(issue("ENG-1", {}))).toBe("#f2c94c");
    expect(linearIssueStateColor(issue("ENG-1"))).toBeNull();
    expect(
      linearIssueStateColor(
        issue("ENG-1", { state: { name: "Done", type: "completed", color: "red;x:y" } }),
      ),
    ).toBeNull();
  });
});
