...
comments and changes. It updates while it's open. Cmd-click (Ctrl-click on Windows and Linux)
opens an issue in Linear instead. On mobile, they appear on the thread row and in the thread's Git sheet.
...
- **Your Linear account (recommended):** with Otter Connect, choose **Link Linear account** under
  **Settings → Connections → Linear agent**. Every machine linked to your Otter account then reads
...
- **A personal API key:** add one in **Settings → Integrations → Linear**, created in Linear under
  **Settings → Security & access**. The key stays on the machine running Otter Code. Reviews
...
4. Choose the project issues run in on that machine: a default project, and optionally a project
   per Linear team. The linked machine's projects appear under your link in **Settings →
   Connections → Linear agent**, and under **Settings → Integrations → Linear** when that machine
   is selected. Projects set on any other machine are not used.
5. Optionally, change the **Linear prompt template** in the same place. It is the thread's first
