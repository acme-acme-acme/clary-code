    ...
    function* (input) {
      ...
      const [row] = yield* db
        ...
        .where(
          and(
            inArray(
              relayLinearUserLinks.userId,
              users.map((user) => user.userId),
            ),
            isNotNull(relayLinearUserLinks.accessTokenSealed),
          ),
        )
        .orderBy(desc(relayLinearUserLinks.updatedAt))
        .limit(1)
        .pipe(Effect.mapError(persistence));
      if (!row?.accessTokenSealed) return null;
      const sealed = row.accessTokenSealed;
      const token = yield* freshToken(
        {
        ...
        (next) =>
          db
            ...
            .where(
              and(
                eq(relayLinearUserLinks.organizationId, row.organizationId),
                eq(relayLinearUserLinks.linearUserId, row.linearUserId),
                eq(relayLinearUserLinks.accessTokenSealed, sealed),
              ),
            )
            .pipe(Effect.mapError(persistence)),
      );
    ...
    function* (userId) {
      ...
      return {
        ...
        links: rows.map((row) => ({
          ...
          signedIn: row.accessTokenSealed !== null,
        })),
      };
    },
  );
